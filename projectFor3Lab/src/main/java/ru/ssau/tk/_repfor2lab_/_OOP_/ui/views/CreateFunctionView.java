...
public class CreateFunctionView extends VerticalLayout {
    ...
    private void createFromPoints(String functionName) {
        try {
            ...
            funcDto.setAmountOfDots((long) xList.size());
            funcDto.setLeftBoarder(Collections.min(xList));
            funcDto.setRightBoarder(Collections.max(xList));
            funcDto.setFunctionType("manual");
            funcDto.setOwnerID((long) userId);
            ...
            List<PointsDTO> points = IntStream.range(0, xList.size())
                    .mapToObj(i -> {
                        ...
                        p.setFunctionID((long) functionId);
                        return p;
                    })
                    .collect(Collectors.toList());
    ...
    private void createFromSimpleFunction(String userProvidedName) {
        try {
            ...
            dto.setAmountOfDots(Long.valueOf(dots));
            dto.setLeftBoarder(left);
            dto.setRightBoarder(right);
            dto.setFunctionType("tabulated");
            dto.setOwnerID((long) userId);
