...
public class MyFunctionView extends VerticalLayout {
    ...
    public MyFunctionView() {
        ...
        var leftCol = grid.addColumn(MathFunctionsDTO::getLeftBoarder).setHeader("Левая граница");
        var rightCol = grid.addColumn(MathFunctionsDTO::getRightBoarder).setHeader("Правая граница");
    ...
    private void openPointsDialog(MathFunctionsDTO func, boolean editable) {
        Optional<List<PointsDTO>> points = fetchPoints(Math.toIntExact(func.getFunctionID()));
        if (points.isEmpty()) {
            return;
        }
    ...
    private void addPoint(MathFunctionsDTO func, Double xValue, Double yValue, List<PointsDTO> points,
        ...
        try {
            ...
            if (response.statusCode() == 201) {
                ...
                fetchPoints(Math.toIntExact(func.getFunctionID())).ifPresent(newPoints -> {
                    points.clear();
